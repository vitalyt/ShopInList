...
final class CoreDataStack: ObservableObject {
  ...
  lazy var persistentContainer: NSPersistentCloudKitContainer = {
    let container = NSPersistentCloudKitContainer(name: "ShopInList")
...
extension CoreDataStack {
  ...
  func delete(_ destination: CDProduct) {
    context.perform {
...
extension CoreDataStack {
  ...
  func getShare(_ product: CDProduct) -> CKShare? {
    guard isShared(object: product) else { return nil }
    guard let shareDictionary = try? persistentContainer.fetchShares(matching: [product.objectID]),
      let share = shareDictionary[product.objectID] else {
      print("Unable to get CKShare")
      return nil
    }
    share[CKShare.SystemFieldKey.title] = product.name
    return share
  }
